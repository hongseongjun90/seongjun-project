...
public class CommunityUpdateCommand implements Command {
  ...
  public void execute() {
    ...
    newCommunity.setId(
        prompt.inputString(String.format("아이디(%s)? ", oldCommunity.getId()), oldCommunity.getId()));

    newCommunity.setNickName(prompt.inputString(
        String.format("닉네임(%s)? ", oldCommunity.getNickName()), oldCommunity.getNickName()));

    newCommunity.setTitle(prompt.inputString(String.format("제목(%s)? ", oldCommunity.getTitle()),
        oldCommunity.getTitle()));
